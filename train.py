...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    downscaling_network = DownscalingNetwork().to(device)
    dfm_optimizer = optim.Adam(downscaling_network.parameters(), lr=0.001) #downscaling_network optimizer 선언
    ...
    ema = ExtendedModelEMA(model,downscaling_network,tau = 2000) if RANK in {-1, 0} else None
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            compressed_size_bits = encode_image_to_memory(resized_imgs, format='JPEG', quality = 10) # downscaled img을 compression 한 후에 bit수(즉, file size)
            ...
            with torch.cuda.amp.autocast(amp): #amp: 자동 혼합 정밀도 -> 그냥 계산 효율성, 성능 향상시키는 것 인듯..
                ...
                dfm_loss = F.mse_loss(resized_imgs2, imgs) #원본 이미지와 upscaled img의 mse로 손실 계산
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size // pred결과와 target 결과를 기반으로 loss 계산
                ...
                total_loss = bpp_loss + 2 * loss #dfm의 loss 함수
            ...
            total_loss.backward(retain_graph=True)#backward(역전파)를 두번 호출하면 처음으로 호출되는 backward에 retain_graph=True 해놓기
            #retain_graph를 사용하는 이유 : dfm 역전파 후에도 계산 그래프를 유지하여 yolov5 모델에 대한 역전파를 수행할 수 있도록 하기 위해서
            # Backward

...
class DownscalingNetwork(nn.Module):#classifcation에서 regrssion으로 변경
    def __init__(self):
        ...
        self.extended_fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.InstanceNorm1d(1000), #인스턴스 정규화 사용 (안정화)
            nn.Dropout(0.5),
            nn.Linear(1000, 256),
            nn.ReLU(),
            nn.InstanceNorm1d(256), #인스턴스 정규화 사용 (안정화)
            nn.Dropout(0.5),
            nn.Linear(256,128),
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        x = (x + 1) / 2 * 3 + 1 #(Tanh 쓸때 [-1,1] -> [1.0,4.0] 로 스케일링)
        return x
...
def run(#train에서 epoch 끝날때마다 시행
    ...
    batch_size=1,  # batch size 32 -> 1로 수정 (default)
    ...
    downscaling_network=None, #0220 추가
    downscaling_network_weights=None #0220 추가
):
    ...
    if training:  # called by train.py train에서 부르는거?
        ...
        downscaling_network = DownscalingNetwork().to(device).half() if half else DownscalingNetwork().to(device).float() #syh edit dfm 로드
        if 'downscaling_network' in ckpt: #0220
            downscaling_network.load_state_dict(ckpt['downscaling_network']) #0220
            print("Loaded downscaling_network weights@@@@@@")#0220
        else:#0220
